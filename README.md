...
**Description:**
This script extracts resume data from a list of URLs provided in a JSON file. It uses the
`requests` library to fetch the HTML content of each resume page and `BeautifulSoup` to parse it.
The data is then saved into individual JSON files for each resume.
